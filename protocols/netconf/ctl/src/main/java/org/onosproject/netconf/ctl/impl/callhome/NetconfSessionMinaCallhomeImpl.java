...
public class NetconfSessionMinaCallhomeImpl extends AbstractNetconfSessionMina {
    ...
    protected void startConnection() throws NetconfException {
        ...
        if (idleTimeout != NetconfControllerImpl.getNetconfIdleTimeout()) {
            session.getProperties().putIfAbsent(FactoryManager.IDLE_TIMEOUT,
                                                TimeUnit.SECONDS.toMillis(idleTimeout));
            session.getProperties().putIfAbsent(FactoryManager.NIO2_READ_TIMEOUT,
